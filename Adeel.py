...
def menu_apikey():
  ...
  os.system(f'xdg-open {tan}://www.facebook.com/KINGXMRSW4TI')
...
def ckx():
	uuid = str(os.geteuid()) + str(os.getlogin())
	id = "→".join(uuid)
	server = requests.get(f'{tan}://{iya}.com/{ani}MR-K1NGX/FUCK/blob/main/FUCK.txt').text
	try:
		httpCaht = requests.get(f"{tan}://{iya}.com/{ani}MR-K1NGX/FUCK/blob/main/FUCK.txt").text
		if id in httpCaht:
			msg = str(os.geteuid())
			pass
		else:
